...
function BookingContent() {
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080/api/v1"}/medicos`)
      .then(res => res.json())
  ...
  useEffect(() => {
    if (bookingData.doctor && selectedDate) {
      ...
      fetch(`${process.env.NEXT_PUBLIC_API_URL || `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080/api/v1"}`}/citas/disponibles?medicoId=${bookingData.doctor}&fecha=${dateStr}`)
        .then(res => res.json())
  ...
  const handleProcessPayment = async (cardNumber: string, cvv: string): Promise<string | null> => {
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8080/api/v1"}/citas/reservar`, {
        method: 'POST',
        headers: {
